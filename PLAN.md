...
- 2026-05-27: **Accessor properties + single inheritance** added to the class API. `.Property(name, getter)` / `.Property(name, getter, setter)` declares accessors backed by `const T&` / `T&` typed lambdas; dispatch goes through a class-level `getProperty` / `setProperty` callback in the JSClass that looks up by name and walks the parent chain so inherited properties resolve. Read-only properties throw on assignment. `.Extends<Parent>(parentCls)` static_asserts `is_base_of_v<Parent, T>`, sets JSC `parentClass` on the instance class (so `JSValueIsObjectOfClass` walks the chain and `EntityCls.Unwrap(playerValue)` yields a correctly-cast `Entity*`), and links `Child.prototype.__proto__ = Parent.prototype` for method-via-prototype inheritance. Each child stores `castToParent` as `static_cast<Parent*>(static_cast<T*>(p))`; `CastUpChain` walks until reaching the owning class, handling base offsets. `instanceof` required a JSClass `hasInstance` callback (JSC's `JSCallbackObject::customHasInstance` returns false by default for callback objects without it) — added an OrdinaryHasInstance-style prototype-chain walk. `examples/classes/` rewritten around `v.x = 10` and an `Entity`/`Player` hierarchy. `test_eval` grew accessor + inheritance assertions (read-write, read-only throw, cross-class `Unwrap` succeeds for parent, fails for child→parent direction, `instanceof` returns true for both classes). Caveats: single inheritance only; cast chain is `static_cast`-based and not safe for virtual inheritance; properties win over methods of the same name. 8/8 tests passing.
